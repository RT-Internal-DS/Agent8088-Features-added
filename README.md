...
| `plan_audit` | `0` | Verify every mutating step with the readonly `auditor` sub-agent — both `execute_plan` steps and the tool calls an approved `/plan` runs, graded against the plan you approved; a failed verification halts the work |
