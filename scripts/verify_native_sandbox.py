...
def _child(workspace: Path, secret: Path, config: Path) -> None:
    ...
    engine._exec_sandbox_command(f"printf allowed > {shlex.quote(str(allowed))}")
    if allowed.read_text(encoding="utf-8") != "allowed":
        _fail("native sandbox could not write the permitted workspace")
