...
# ---------------------------------------------------------------------------
# _ThrottledLive — repaint only on real change, and never mid-frame
# ---------------------------------------------------------------------------
class _FakeLive:
    """Stands in for rich.live.Live: records what it was asked to draw."""
    ...
    def refresh(self):
        self.refreshes += 1
        cli.console.print(self.renderable)


def test_throttled_live_draws_the_body_untouched(monkeypatch):
    """Nothing is appended to the frame. Drawing the status bar here was tried
    and reverted: Live sits at the bottom of the output, not of the terminal, so
    the bar surfaced halfway up the screen beside the spinner."""
    output = io.StringIO()
    monkeypatch.setattr(cli, "console",
                        Console(file=output, width=100, height=24, color_system=None,
                                legacy_windows=False))
    fake = _FakeLive()
    live = cli._ThrottledLive(fake)

    live.update(cli.Text("streaming answer"))
    live._paint()

    rendered = output.getvalue()
    assert "streaming answer" in rendered
    assert "8088" not in rendered and "ctx" not in rendered


def test_throttled_live_repaints_only_when_something_changed():
    """Rich's own refresh thread repaints unconditionally; that churn is the
    flicker. Only new content, or a spinner owed a tick, may schedule a frame."""
    live = cli._ThrottledLive(_FakeLive())

    live.update(cli.Text("prose"))
    assert live._dirty is True, "new content must schedule a repaint"

    live._body, live._dirty = cli.Text("prose"), False
    assert cli._ThrottledLive._animates(live._body) is False, \
        "a static prose panel must not schedule a repaint on its own"

    spinner = cli._StatusLine("thinking", 0.0, [0], interruptible=True)
    assert cli._ThrottledLive._animates(spinner) is True, \
        "the spinner still has to animate while the model is quiet"


def test_throttled_live_brackets_each_frame_in_synchronized_output(monkeypatch):
    """Rich emits no synchronized-output guard, so an erase-then-redraw can be
    presented half-finished. These two sequences are what stop that."""
    output = io.StringIO()
    console = Console(file=output, width=100, height=24, color_system=None,
                      legacy_windows=False, force_terminal=True)
    monkeypatch.setattr(cli, "console", console)
    live = cli._ThrottledLive(_FakeLive())

    live.update(cli.Text("frame"))
    live._paint()

    rendered = output.getvalue()
    assert rendered.index("\x1b[?2026h") < rendered.index("frame")
    assert rendered.index("frame") < rendered.index("\x1b[?2026l")


def test_throttled_live_skips_painting_while_the_live_is_stopped(monkeypatch):
    """_handle_escalation stops the Live to ask a question on a clean screen;
    painting under it would overwrite the prompt."""
    output = io.StringIO()
    monkeypatch.setattr(cli, "console",
                        Console(file=output, width=100, height=24, color_system=None,
                                legacy_windows=False))
    fake = _FakeLive()
    fake.is_started = False
    live = cli._ThrottledLive(fake)
    live.update(cli.Text("should not appear"))

    live._stop.set()          # so _run exits after one pass
    live._run()
