...
**Two additional design choices worth calling out explicitly:**

- An itemized "here's what this will remove" preview before the confirmation prompt, and a `--dry-run` flag that prints the same list without touching anything. Both are cheap given the plan already has functions that *detect* what needs removing — see Task 10.
- Whether user data (trace logs, WhatsApp session) should be removed by default or kept by default is a genuine judgment call, not an obvious one — see Design Decision 6 and the open question at the end.
...
6. **Full removal is the default, not opt-in.** This plan keeps full-removal-by-default with `--keep-data` as the opt-out, because that's what was explicitly asked for ("everything needs to be uninstalled to the state before... installation"). The opposite stance — preserve config/data by default, require a flag or manual step for full deletion — is a real, defensible alternative and is flagged again in the open questions at the end.
7. **Preview and confirm before deleting.** `--uninstall` should show exactly what it found (which PATH lines, which cron/scheduled-task entries, which directories) before asking for confirmation, and support a `--dry-run` flag that prints the same list without touching anything. See Task 10.
...
**Objective:** Show the user exactly what was found (PATH lines, cron/scheduled-task entries, data directories) before asking them to confirm, and let them preview the same list without deleting anything via `--dry-run`. This reuses the detection logic Tasks 4-9 already wrote; it does not duplicate it.
...
4. **Confirm before implementing:** should `--uninstall` (no flags) really default to deleting trace logs and the WhatsApp session by default, or should it default to *keeping* user data and require an explicit `--purge-data` to remove it? This plan chose "delete by default, `--keep-data` opts out" to match the literal ask ("everything needs to be uninstalled to the state before... installation"), but it's worth a second opinion before shipping, since it's the one part of this plan that deletes user-generated content rather than program artifacts.
5. **Considered and left out:** a fully granular per-component flag set (separate flags for the shim, the PATH entries, scheduled tasks, etc.) and requiring the user to type the install path or a confirmation phrase (rather than just "yes") were both considered. agent8088's uninstall surface is small enough today (one install dir, no separate service/app to target) that granular component flags would be premature, and "type yes" already matches the rest of this codebase's existing confirmation style. Revisit if agent8088 grows a separate long-running service component later.
