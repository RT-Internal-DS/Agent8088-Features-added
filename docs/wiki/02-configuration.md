...
| `browser_max_steps` | Max steps `browse_page`'s browsing agent takes on one task (default `500`, matching Browser Use). The wall-clock timeout remains the hard bound. |
| `browser_task_timeout_seconds` | Overall wall-clock limit for one `browse_page` call (default `600`). Clamped to `max_tool_timeout_seconds`, the hard ceiling for any single tool call. |
