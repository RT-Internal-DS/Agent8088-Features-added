...
This is a live API key for the Ornith-35B model on `192.168.3.67:8080`. It's currently tracked by git (no `.gitignore` entry for `config.txt`). If this repo ever goes public, the key leaks.
