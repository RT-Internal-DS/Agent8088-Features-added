...
export function ToolChip({ name, status, result }: ToolChipProps) {
  ...
  return (
    <div className="my-0.5 rounded-lg border border-zinc-800/60 bg-zinc-900/30 text-[13px]">
      <button
      ...
      {expanded && result && (
        <pre className="max-h-40 overflow-auto border-t border-zinc-800/60 p-2 font-mono text-[11px] text-zinc-400">
          {result}
        </pre>
      )}
    </div>
